...
#dota0.py - scroll(autopick),
#check buttons on main menu
#(запустить все процессы(запускает все скрипты)
#завершить все процессы(завершает все скрипты, все настройки как при запуск делает)
#включить программу(только те где стояло во вкладках "включено", так же запускает с циклами, установленными(для autopick, попутно с выбранными героями))
#выключить программу(завершает те скрипты, которые были включены, и меняет состояние на главном с "В работе" на "выключено" и меняет во вкладках все настройки до того, как было изначально)
#)
#Так же работоспособность каждого скрипта
#(кнопка random.png перенесена в icons\pictures
#Создай на данный момент heroes.txt в constants и засунь из heroes.py(то что зеленым) в этот txt